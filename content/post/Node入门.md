...
title = "Node入门"
draft = false
date = "2016-04-25T17:39:54+08:00"
...
购买“Node入门”中文版电子书----->[立即购买](http://www.nodebeginner.org/buy-chinese/)
...
这里要记住的是，浏览器发出请求后获得并显示的“Hello World”信息仍是来自于我们server.js文件中的`onRequest`函数。
...
和往常一样，我们启动服务器，然后访问http://localhost:8888/start 。
...
这里我们要用到的外部模块是Felix Geisendörfer开发的node-formidable模块。它对解析上传的文件数据做了很好的抽象。 其实说白了，处理文件上传就是处理POST数据 —— 但是，麻烦的是在具体的处理细节，所以，这里采用现成的方案更合适点。
